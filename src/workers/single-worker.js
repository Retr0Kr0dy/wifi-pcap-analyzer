...
self.onmessage = async function(e) {
    const { ssid, bssid, sta, anonce, snonce, micFrames, wordlist } = e.data;
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (triedCount % 50 === 0)
            self.postMessage({ type:'progress', tried:triedCount, total, elapsed:(Date.now()-t0)/1000 });
    }

    self.postMessage({ type:'done', tried:triedCount, total, elapsed:(Date.now()-t0)/1000 });
};
